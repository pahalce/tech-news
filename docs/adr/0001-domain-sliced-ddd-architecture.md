...
`Agent State` owns persistence shape, schema versioning, JSON file mapping, and the final data commit. Each domain module owns its state slice types, invariants, and update rules. Workflows compose these slices and persist the resulting state, but they do not embed domain rules.

Boundary rules should be enforced by the existing vite-plus lint entrypoint, `vp lint`, using `.oxlintrc.json` for import restrictions. A small architecture check may supplement oxlint for source-dependent layer rules that `no-restricted-imports` cannot express cleanly.
